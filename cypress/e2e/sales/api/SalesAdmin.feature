Feature: Sales Management API
  ...
  Scenario: POST Create Sale - Valid Request (API_Ad_01_214025B)
    Given I have a payload for a new sale with Plant ID 1 and Quantity 1
    When I send a POST request to "/api/sales/plant/{id}"
    Then the response status code should be 201 s2
  ...
  Scenario: Verify Unauthorized Access (API_Ad_05_214025B)
    ...
    Then the response status code should be 401 s2