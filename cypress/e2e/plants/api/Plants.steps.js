  ...
  if (createdPlantId) {
    ...
    deletePlantById(createdPlantId, headers).then((res) => {
      if (res.status === 200 || res.status === 204) {
        cy.log(
          `✓ Cleanup: Plant with ID ${createdPlantId} deleted successfully`,
        );
      } else {
        cy.log(
          `⚠ Cleanup: Failed to delete plant ID ${createdPlantId}, Status: ${res.status}`,
        );
      }
    });
  }
...
Given("I have a valid admin authentication token", () => {
  loginAdmin().then((res) => {
    if (res.status === 200 && res.body.token) {
      ...
      cy.log(
        "Proceeding without authentication token (API may not require auth)",
      );
      authToken = "";
    }
  });
...
Given(
  "I have plant details with name {string} and price {float} and quantity {int}",
  (name, price, quantity) => {
    const headers = getHeaders(authToken);
...
When(
  "I send a POST request to create a plant under category ID {int}",
  (categoryId) => {
    const headers = getHeaders(authToken);
...
When(
  "I send a PUT request to update plant ID {int} with name {string} and price {float}",
  (plantId, newName, newPrice) => {
    const headers = getHeaders(authToken);
    const updateBody = {
      id: plantId,
      name: newName,
...
When(
  "I send a PUT request to update plant ID {int} with name {string}, price {float}, and quantity {int}",
  (plantId, name, price, quantity) => {
    const headers = getHeaders(authToken);
...
When(
  "I send a GET request to retrieve plants filtered by category ID {int}",
  (categoryId) => {
    const headers = getHeaders(authToken);
    cy.request({
      method: "GET",
...
Then("the response status code should be {int} or {int}", (code1, code2) => {
  ...
  cy.log(
    `✓ Status code ${response.status} matches expected (${code1} or ${code2})`,
  );
});
...
Then(
  "the response body should contain the created plant with a generated ID",
  () => {
...
Then(
  "the response body should contain a validation error message {string}",
  (expectedMessage) => {
    expect(response.body).to.not.be.null;
...
Then(
  "the response body should confirm the update with name {string} and price {float}",
...
Then(
  "the response body should only contain plants in category ID {int}",
  (expectedCategoryId) => {
    expect(response.body).to.be.an("array");

    const filtered = response.body.filter(
      (plant) => plant.category && plant.category.id,
    );
    filtered.forEach((plant) => {
      expect(plant.category.id).to.eq(expectedCategoryId);
    });
    cy.log(
      `✓ All filtered plants belong to category ID: ${expectedCategoryId}`,
    );
  },
);
